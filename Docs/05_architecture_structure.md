...
* `server` — серверное приложение с API и прикладной логикой;
...
* `api` — HTTP-контроллеры, входные и выходные модели, преобразование ошибок;
* `application` — прикладные сценарии и границы транзакций;
* `domain` — сущности, инварианты и интерфейсы репозиториев;
* `infrastructure` — работа с БД, безопасность, конфигурация и интеграция с фреймворком.
...
* бизнес-правила задаются в `03_domain_model.md`;
...
* smoke-тесты ключевых сценариев API.
